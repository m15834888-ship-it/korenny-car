              ...
              {navItems.map(item => {
                ...
                return (
                  <button key={item.path} onClick={() => navigate(item.path)}
                    className="relative flex flex-col items-center gap-1.5 py-3 px-6 rounded-2xl transition-all">
                    {isActive && (
                      <motion.div layoutId="nav-indicator"
                        className="absolute -top-2 w-12 h-1.5 rounded-full"
                        style={{ background: 'linear-gradient(90deg, #D4AF37, #B08D2A)' }} />
                    )}
                    <div className="relative">
                      <Icon className="w-6 h-6" style={{ color: isActive ? '#D4AF37' : '#8888a0' }} />
                      {item.badge ? (
                        <span className="absolute -top-2.5 -right-2.5 w-6 h-6 rounded-full text-[10px] font-bold flex items-center justify-center"
                          style={{ background: '#D4AF37', color: '#0c0c0e' }}>
                          {item.badge}
                        </span>
                      ) : null}
                    </div>
                    <span className="text-xs font-semibold" style={{ color: isActive ? '#D4AF37' : '#8888a0' }}>
                      {item.label}
                    </span>
                  </button>
                );
