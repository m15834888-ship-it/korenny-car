...
export default function AdminCarsPage() {
  ...
  const [form, setForm] = useState({
    name: '', brand: '', type: 'Sedan' as Car['type'], price: '', year: '', description: '', image_url: '',
  });

  const resetForm = () => {
    setForm({ name: '', brand: '', type: 'Sedan', price: '', year: '', description: '', image_url: '' });
    setEditId(null);
    setShowAdd(false);
  };
  ...
  const handleSave = () => {
    ...
    const carData = {
      ...
      image_url: form.image_url || 'https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=600&h=400&fit=crop',
  ...
  const startEdit = (car: Car) => {
    setForm({
      ...
      price: String(car.price), year: String(car.year),
    ...
    <div className="pb-24 px-4 pt-4">
      ...
      {showAdd && (
        <motion.div
          ...
          style={{ background: 'rgba(20, 20, 24, 0.9)', border: '1px solid rgba(212, 175, 55, 0.2)' }}
        >
          <input placeholder={t('admin.carName')} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            className="w-full py-2.5 px-3 rounded-lg text-sm outline-none" style={{ background: 'rgba(42, 42, 48, 0.6)', border: '1px solid rgba(212, 175, 55, 0.15)', color: '#e0e0e5' }} />
          <div className="grid grid-cols-2 gap-3">
            <input placeholder={t('admin.carBrand')} value={form.brand} onChange={e => setForm(f => ({ ...f, brand: e.target.value }))}
              className="py-2.5 px-3 rounded-lg text-sm outline-none" style={{ background: 'rgba(42, 42, 48, 0.6)', border: '1px solid rgba(212, 175, 55, 0.15)', color: '#e0e0e5' }} />
          ...
          <textarea placeholder={t('admin.carDesc')} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            className="w-full py-2.5 px-3 rounded-lg text-sm outline-none resize-none h-20" style={{ background: 'rgba(42, 42, 48, 0.6)', border: '1px solid rgba(212, 175, 55, 0.15)', color: '#e0e0e5' }} />
          <div className="flex gap-2">
            <button onClick={handleSave} className="flex-1 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-1"
              style={{ background: 'linear-gradient(135deg, #D4AF37, #B08D2A)', color: '#0c0c0e' }}>
              <Check className="w-4 h-4" /> {t('admin.save')}
            </button>
            <button onClick={resetForm} className="px-4 py-2.5 rounded-lg text-sm" style={{ background: 'rgba(42, 42, 48, 0.6)', color: '#8888a0' }}>
              {t('admin.cancel')}
            </button>
          </div>
        </motion.div>
      ...
      {pendingOrders.length > 0 && (
        <div className="mb-4">
          <h2 className="text-sm font-semibold mb-2 flex items-center gap-2" style={{ color: '#D4AF37' }}>
            <Clock className="w-4 h-4" /> {t('admin.pendingOrders')} ({pendingOrders.length})
          </h2>
          {pendingOrders.map(order => (
            <div key={order.id} className="rounded-xl p-3 mb-2" style={{ background: 'rgba(20, 20, 24, 0.8)', border: '1px solid rgba(212, 175, 55, 0.1)' }}>
              <div className="flex items-center gap-2 mb-1">
                <User className="w-3.5 h-3.5" style={{ color: '#8888a0' }} />
              ...
              <button onClick={() => confirmOrder(order.id)} className="px-3 py-1.5 rounded-lg text-[10px] font-bold"
      ...
      <div className="space-y-2">
        {cars.map(car => (
          <div key={car.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(20, 20, 24, 0.8)', border: '1px solid rgba(212, 175, 55, 0.1)' }}>
            <img src={car.image_url} alt={car.name} className="w-14 h-10 rounded-lg object-cover" />
            ...
            <select
              value={car.status}
              onChange={e => updateCar(car.id, { status: e.target.value as Car['status'] })}
              className="py-1 px-2 rounded-lg text-[10px] outline-none"
            ...
            <button onClick={() => startEdit(car)} className="p-1.5 rounded-lg" style={{ background: 'rgba(212, 175, 55, 0.1)' }}>
              <Edit className="w-3.5 h-3.5" style={{ color: '#D4AF37' }} />
            </button>
            <button onClick={() => deleteCar(car.id)} className="p-1.5 rounded-lg" style={{ background: 'rgba(239, 68, 68, 0.1)' }}>
