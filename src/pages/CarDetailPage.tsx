  ...
  return (
    <div className="pb-24">
      ...
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 w-10 h-10 rounded-full flex items-center justify-center"
        ...
        <button
          ...
          style={{
            ...
            [document.documentElement.dir === 'rtl' ? 'left' : 'right']: '16px',
          }}
        >
        ...
        {/* Status */}
        <span
          className="absolute bottom-4 px-3 py-1 rounded-full text-xs font-bold"
          style={{
            background: car.status === 'available' ? 'rgba(34, 197, 94, 0.9)' : 'rgba(239, 68, 68, 0.9)',
            color: '#fff',
            [document.documentElement.dir === 'rtl' ? 'right' : 'left']: '16px',
          }}
        >
      ...
      <div className="px-4 -mt-6 relative z-10">
        ...
        >
          <h1 className="text-xl font-bold mb-1" style={{ color: '#D4AF37' }}>{car.name}</h1>
          <p className="text-2xl font-bold mb-4" style={{ color: '#e0e0e5' }}>
            {car.price.toLocaleString()} <span className="text-sm font-normal" style={{ color: '#8888a0' }}>{t('home.price')}</span>
          </p>

          {/* Specs */}
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="flex items-center gap-2 p-3 rounded-xl" style={{ background: 'rgba(42, 42, 48, 0.5)' }}>
